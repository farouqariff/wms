  ...
  const fetchPageData = async () => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/pages/operator/",
        {
          credentials: "include",
        }
      );
      ...
      <div className="max-w-4xl mx-auto py-8">
        ...
        <div className="bg-slate-800 rounded-xl shadow-2xl p-8 border border-slate-700 text-center">
          ...
          <p className="text-slate-400 mb-6">
            You have operator-level access to the system.
          </p>

          <div className="bg-slate-700 rounded-lg p-4 mt-6">
            <p className="text-slate-300 text-sm">
              Role:{" "}
              <span className="text-orange-400 font-semibold">Operator</span>
            </p>
            <p className="text-slate-300 text-sm mt-2">
              Permissions: View Operator Page
            </p>
          </div>
        </div>
      </div>
